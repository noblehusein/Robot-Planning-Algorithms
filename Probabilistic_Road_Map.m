clc;
clear all;
image_Map=im2bw(imread('map4.bmp')); % input map read from a bmp file. for new maps write the file name here
...
function feasible=PointPossible(point,map)
feasible=true;
...
function feasible=PathCheck(n,newPos,map)
feasible=true;
dir=atan2(newPos(1)-n(1),newPos(2)-n(2));
for r=0:0.5:sqrt(sum((n-newPos).^2))
    posCheck=n+r.*[sin(dir) cos(dir)];
    if ~(PointPossible(ceil(posCheck),map) && PointPossible(floor(posCheck),map) && ... 
            PointPossible([ceil(posCheck(1)) floor(posCheck(2))],map) && PointPossible([floor(posCheck(1)) ceil(posCheck(2))],map))
        feasible=false;break;
    end
    if ~PointPossible(newPos,map), feasible=false; end
end
end
